...
- `MODE.txt` holds one word: `draft` (a post publishes ONLY when its Status is **Approved** on the weekly approval sheet), `silent` (publishes unless Status is Veto/Edit by its publish time), `auto` (publish; weekly audit). **Starts in `draft`; kick-off = Monday 14 September 2026 (after Yom Teruah); review the mode on 2026-09-28.**
...
**Publisher — daily 09:30 SAST.** 1) Check `PAUSED`, `MODE.txt`, `NO_POST_DAYS.txt` (and Saturday). 2) Find this week's approval sheet in Drive (title "Engine — Week of <this week's Monday>"), read it, save the returned table to a temp file, run `node scripts/marketing-sheet.mjs --apply <file>` so reviewer edits and statuses land in the specs. 3) `node scripts/marketing-sheet.mjs --today`. If there is no spec for today (and today is a posting day), create one from CALENDAR + BACKLOG following §2–§6, render its card, save it as draft — it will be reviewed on next week's sheet, so it does NOT publish today. 4) Publish today's spec only if its status is `approved` (in `silent` mode: also if `draft` and not vetoed/edited): `node scripts/marketing-publish.mjs --post "<spec>"` → set `status: published`, log. 5) Append one line to LOG.md either way.
**Editor + Analyst — Mondays 07:00 SAST.** 1) Draft the FOLLOWING week's 6 specs + cards into QUEUE (Mon–Sun starting next Monday; never overwrite existing; skip NO_POST_DAYS). 1b) Build that week's approval sheet: `node scripts/marketing-sheet.mjs --csv --week <next Monday>` → create a Google Sheet in the "Engine — For approval" Drive folder titled "Engine — Week of <next Monday> (approve here)"; `node scripts/marketing-sheet.mjs --copy-cards --week <next Monday>`; add the HOW-TO row. The digest carries the sheet link and a one-line message Konna can forward to Chantal. 2) Draft one guide into `GUIDES/` (Q&A-formatted, GEO-friendly, from BACKLOG topics; stats only from §2). 3) Numbers: `node scripts/set-referral-code.mjs --list` (sign-ups by code) + `--stats` if present; read last week's LOG. 4) Write `DIGEST-<date>.md` in this folder: what went out, what's queued (with the copy — Konna approves by replying in chat), numbers, and the **human-only asks** for the week with the text pre-written (WhatsApps from the Launch Kit, a Facebook-group post from Asset Pack 2, a video script from Asset Pack 6, press pitch when relevant). Keep it to one screen. The run's completion notification carries the digest to Konna's inbox.
